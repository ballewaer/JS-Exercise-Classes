...
class Car {
  ...
  drive(distance) {
    let mileage = this.tank * this.milesPerGallon;
...
class Lambdasian {
  constructor(lamAttr) {
    this.name = lamAttr.name;
    this.age = lamAttr.age;
    this.location = lamAttr.location;
  }
  speak() {
...
class Instructor extends Lambdasian{
  ...
  demo(subject) {
    return `Todaywe are learning about ${subject}`;
  }
  grade(student, subject) {
    return `${subject.name} receives a perfect  score on ${subject}`;
  }
}
...
class Student extends Lambdasian{
  constructor(stuAttr) {
    ...
    sprintChallenge(subject) {
      return `${this.name} has beun sprint challenge on ${subject}`;
    }
}
